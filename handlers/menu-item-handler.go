...
func CreateMenuItemHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(item.Ingredients) > 1 {
		var id_ingredients []int
		for i := 0; i < len(item.Ingredients); i++ {
			for j := 0; j < len(id_ingredients); j++ {
				if id_ingredients[j] == item.Ingredients[i].IngredientID {
					http.Error(w, "Ingredietns must be unique", http.StatusBadRequest)
					return
				}
			}
			id_ingredients = append(id_ingredients, item.Ingredients[i].IngredientID)
		}
	}
...
func UpdateMenuItemHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(item.Ingredients) < 1 {
		http.Error(w, "Ingredients count must be more than 0", http.StatusBadRequest)
		return
	}
